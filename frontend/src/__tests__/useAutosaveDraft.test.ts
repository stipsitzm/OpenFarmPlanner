...
describe('useAutosaveDraft', () => {
  ...
  it('creates nested objects when using dot-path setField', () => {
    const initialData: TestData = { name: 'Test', value: 10 };
    const validate = (): ValidationResult => ({ isValid: true, errors: {} });
    const save = vi.fn();
    ...
    act(() => {
      result.current.setField('meta.details.label', 'Leaf');
    });

    expect((result.current.draft.meta as Record<string, unknown>).details).toEqual({ label: 'Leaf' });
  });

  it('registers beforeunload warning only when draft is dirty and valid', async () => {
    const initialData: TestData = { name: 'Test', value: 10 };
    const validate = (): ValidationResult => ({ isValid: true, errors: {} });
    const save = vi.fn();
  ...
  it('does not set beforeunload warning when invalid or pristine', () => {
    const initialData: TestData = { name: '', value: 10 };
    const validate = (draft: TestData): ValidationResult => ({
      isValid: Boolean(draft.name),
      errors: draft.name ? {} : { name: 'required' },
    });
    const save = vi.fn();
    ...
    // invalid + pristine
    const e1 = new Event('beforeunload') as BeforeUnloadEvent;
    Object.defineProperty(e1, 'returnValue', { configurable: true, writable: true, value: '' });
    window.dispatchEvent(e1);
    expect(e1.returnValue).toBe('');

    // valid + pristine
    act(() => {
      result.current.setField('name', 'Now valid');
      result.current.resetDraft();
    });

    const e2 = new Event('beforeunload') as BeforeUnloadEvent;
    Object.defineProperty(e2, 'returnValue', { configurable: true, writable: true, value: '' });
    window.dispatchEvent(e2);
    expect(e2.returnValue).toBe('');
  });

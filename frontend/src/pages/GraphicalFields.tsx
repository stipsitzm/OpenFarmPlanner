...
const snapToNeighbors = (
  ...
  neighbors.filter((neighbor) => neighbor.id !== currentId).forEach((neighbor) => {
...
export default function GraphicalFields({ showTitle = true }: GraphicalFieldsProps): React.ReactElement {
  ...
  const [stageWidth, setStageWidth] = useState<number>(() => Math.max(320, window.innerWidth - 48));
  const [stageHeight, setStageHeight] = useState<number>(() => Math.max(MIN_STAGE_HEIGHT, Math.min(MAX_STAGE_HEIGHT, Math.round(window.innerHeight * 0.45))));
  const [activeGuides, setActiveGuides] = useState<GuideLine[]>([]);
  const [interactionMode, setInteractionMode] = useState<InteractionMode>('view');
  const [viewportByLocation, setViewportByLocation] = useState<Record<number, ViewportState>>({});
  ...
  useEffect(() => {
    const handleResize = (): void => {
      ...
      setStageHeight(Math.max(MIN_STAGE_HEIGHT, Math.min(MAX_STAGE_HEIGHT, Math.round(window.innerHeight * 0.45))));
    };
  ...
  useEffect(() => {
    const expandedIds = Object.entries(expanded).filter(([, isExpanded]) => isExpanded).map(([locationId]) => Number(locationId));
  ...
  const saveFieldLayout = (locationId: number, payload: FieldLayoutEntry): void => {
  ...
  const locationLayouts = useMemo(() => {
    return locations.flatMap((location) => {
      ...
      const contentWidth = Math.max(stageWidth, ...defaultFieldRects.map((rect) => rect.width + rect.defaultX + padding));
      const fieldViewModels: RectViewModel[] = defaultFieldRects.map((baseRect) => {
        ...
        const clamped = clampInsideParent(
          { x: savedFieldLayout?.x ?? baseRect.defaultX, y: savedFieldLayout?.y ?? baseRect.defaultY },
          { width: baseRect.width, height: baseRect.height },
          { width: contentWidth, height: contentHeight },
        );

        return { id: fieldId, x: clamped.x, y: clamped.y, width: baseRect.width, height: baseRect.height };
      });

      return [{ location, defaultFieldRects, fieldViewModels, contentWidth, contentHeight }];
    });
  ...
  const resetViewport = (locationId: number): void => {
    ...
    setViewportByLocation((prev) => ({
      ...prev,
      [locationId]: fitContentToStage({ width: layout.contentWidth, height: layout.contentHeight }, { width: stageWidth, height: stageHeight }, VIEWPORT_PADDING),
    }));
  };
  ...
  useEffect(() => {
    setViewportByLocation((prev) => {
      ...
      locationLayouts.forEach((layout) => {
        ...
        next[locationId] = fitContentToStage({ width: layout.contentWidth, height: layout.contentHeight }, { width: stageWidth, height: stageHeight }, VIEWPORT_PADDING);
      });
      return next;
    });
  }, [locationLayouts, stageHeight, stageWidth]);

  const updateViewport = (locationId: number, updater: (current: ViewportState) => ViewportState): void => {
    setViewportByLocation((prev) => {
      const current = prev[locationId] ?? fitContentToStage({ width: stageWidth, height: stageHeight }, { width: stageWidth, height: stageHeight }, VIEWPORT_PADDING);
      return { ...prev, [locationId]: updater(current) };
    });
  };

  const handleZoom = (locationId: number, factor: number): void => {
    updateViewport(locationId, (current) => zoomAroundPoint(current, current.scale * factor, { x: stageWidth / 2, y: stageHeight / 2 }));
  };

  const handleStageWheel = (locationId: number, event: KonvaEventObject<WheelEvent>): void => {
    ...
    updateViewport(locationId, (current) => zoomAroundPoint(current, current.scale * direction, pointer));
  };
  ...
  const handleStageTouchMove = (locationId: number, event: KonvaEventObject<TouchEvent>): void => {
    ...
    updateViewport(locationId, (current) => {
      const zoomed = zoomAroundPoint(current, current.scale * scaleFactor, center);
      return { ...zoomed, x: zoomed.x + (center.x - lastPinch.center.x), y: zoomed.y + (center.y - lastPinch.center.y) };
    });
  ...
  return (
    <Box p={3} ref={containerRef}>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} spacing={2} sx={{ mb: 2 }}>
        <Box>
          {showTitle ? <Typography variant="h4" gutterBottom>{t('fields:graphical.title')}</Typography> : null}
      ...
      {locationLayouts.map(({ location, defaultFieldRects, fieldViewModels, contentWidth, contentHeight }) => {
        if (!location.id) {
          return null;
        }

        const locationId = location.id;
        const viewport = viewportByLocation[locationId] ?? fitContentToStage({ width: contentWidth, height: contentHeight }, { width: stageWidth, height: stageHeight }, VIEWPORT_PADDING);
        const visibility = getVisibleElements(viewport.scale);

        return (
          <Accordion key={locationId} expanded={Boolean(expanded[locationId])} onChange={(_, isExpanded) => setExpanded((prev) => ({ ...prev, [locationId]: isExpanded }))}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6">{t('fields:graphical.locationTitle', { name: location.name })}</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Stack direction="row" spacing={1} sx={{ mb: 1, flexWrap: 'wrap' }}>
                <Button size="large" variant="outlined" startIcon={<AddIcon />} onClick={() => handleZoom(locationId, ZOOM_STEP)} aria-label={t('fields:graphical.zoomIn')}>
                  {t('fields:graphical.zoomIn')}
                </Button>
                <Button size="large" variant="outlined" startIcon={<RemoveIcon />} onClick={() => handleZoom(locationId, 1 / ZOOM_STEP)} aria-label={t('fields:graphical.zoomOut')}>
                  {t('fields:graphical.zoomOut')}
                </Button>
                <Button size="large" variant="outlined" startIcon={<FitScreenIcon />} onClick={() => resetViewport(locationId)} aria-label={t('fields:graphical.fitToView')}>
                  {t('fields:graphical.fitToView')}
                </Button>
                <Typography variant="body2" color="text.secondary" sx={{ alignSelf: 'center', ml: 1 }}>
                  {t('fields:graphical.zoomLevel', { value: viewport.scale.toFixed(2) })}
                </Typography>
              </Stack>
              <Box sx={{ width: '100%', overflow: 'hidden', borderRadius: 2, border: '1px solid', borderColor: 'divider', touchAction: 'none' }}>
                <Stage
                  width={stageWidth}
                  height={stageHeight}
                  ...
                  onTouchMove={(event) => handleStageTouchMove(locationId, event)}
                  ...
                  <Layer>
                    {defaultFieldRects.map((baseRect) => {
                      ...
                      const fieldInnerSize = {
                        ...
                        height: baseRect.height - FIELD_INNER_OFFSET_Y - FIELD_INNER_BOTTOM_PADDING,
                      };
                      fieldBeds.forEach((bed) => {
                        bedSizeMap.set(bed.id!, getBedRectSizeWithinField(bed, baseRect.field, fieldInnerSize));
                      });

                      const missingBeds = fieldBeds.map((bed) => bed.id!).filter((bedId) => !layoutsByBed[bedId]);
                      ...
                      const bedViewModels: BedViewModel[] = fieldBeds.map((bed) => {
                        ...
                        const fallback = autoLayout.get(bedId) ?? { x: 10, y: 10 };
                        const clamped = clampInsideParent({ x: saved?.x ?? fallback.x, y: saved?.y ?? fallback.y }, size, fieldInnerSize);
                        return { id: bedId, name: bed.name, area: Number(bed.area_sqm ?? 0), x: clamped.x, y: clamped.y, width: size.width, height: size.height };
                      });
                      ...
                      return (
                        <Group key={fieldId}>
                          <Rect
                            ...
                            onClick={() => {
                              if (interactionMode === 'view') {
                                setSelectedElement({ type: 'field', name: baseRect.field.name, area: Number(baseRect.field.area_sqm ?? 0), locationName: location.name });
                              }
                            }}
                            onTap={() => {
                              if (interactionMode === 'view') {
                            ...
                            onDragMove={(event: KonvaEventObject<Event>) => {
                              ...
                              const clampedRaw = clampInsideParent(raw, { width: baseRect.width, height: baseRect.height }, { width: contentWidth, height: contentHeight });
                              const snapped = snapToNeighbors(fieldId, clampedRaw, { width: baseRect.width, height: baseRect.height }, fieldViewModels, FIELD_SNAP_THRESHOLD);
                              const finalClamped = clampInsideParent({ x: snapped.x, y: snapped.y }, { width: baseRect.width, height: baseRect.height }, { width: contentWidth, height: contentHeight });
                            ...
                            onDragEnd={(event: KonvaEventObject<Event>) => {
                              ...
                              const next = clampInsideParent({ x: event.target.x(), y: event.target.y() }, { width: baseRect.width, height: baseRect.height }, { width: contentWidth, height: contentHeight });
                              const snapped = snapToNeighbors(fieldId, next, { width: baseRect.width, height: baseRect.height }, fieldViewModels, FIELD_SNAP_THRESHOLD);
                              const finalClamped = clampInsideParent({ x: snapped.x, y: snapped.y }, { width: baseRect.width, height: baseRect.height }, { width: contentWidth, height: contentHeight });
                              event.target.position(finalClamped);
                              const nextLayout: FieldLayoutEntry = { field: fieldId, location: locationId, x: finalClamped.x, y: finalClamped.y, version: 1 };
                              setActiveGuides([]);
                              setLayoutsByField((prev) => ({ ...prev, [fieldId]: nextLayout }));
                              saveFieldLayout(locationId, nextLayout);
                            }}
                          />
                          {shouldShowFieldLabel({ width: baseRect.width, height: baseRect.height }, viewport.scale) ? (
                            <Text
                          ...
                          {visibility.showBeds ? bedViewModels.map((bedVm) => (
                            <Group key={bedVm.id}>
                              <Rect
                                x={currentFieldRect.x + FIELD_INNER_OFFSET_X + bedVm.x}
                                y={currentFieldRect.y + FIELD_INNER_OFFSET_Y + bedVm.y}
                                width={bedVm.width}
                                height={bedVm.height}
                                fill="#bee3f8"
                                stroke="#2b6cb0"
                                ...
                                onClick={() => {
                                  if (interactionMode === 'view') {
                                    setSelectedElement({ type: 'bed', name: bedVm.name, area: bedVm.area, locationName: location.name, parentName: baseRect.field.name });
                                  }
                                }}
                                onTap={() => {
                                  if (interactionMode === 'view') {
                                ...
                                onDragMove={(event: KonvaEventObject<Event>) => {
                                  ...
                                  const snapped = snapToNeighbors(bedVm.id, clampedRaw, { width: bedVm.width, height: bedVm.height }, bedViewModels);
                                  const finalClamped = clampInsideParent({ x: snapped.x, y: snapped.y }, { width: bedVm.width, height: bedVm.height }, fieldInnerSize);
                                  event.target.position({ x: currentFieldRect.x + FIELD_INNER_OFFSET_X + finalClamped.x, y: currentFieldRect.y + FIELD_INNER_OFFSET_Y + finalClamped.y });
                                  setActiveGuides(snapped.guides.map((guide) => ({
                                    ...guide,
                                    value: guide.orientation === 'vertical' ? currentFieldRect.x + FIELD_INNER_OFFSET_X + guide.value : currentFieldRect.y + FIELD_INNER_OFFSET_Y + guide.value,
                                    start: guide.orientation === 'vertical' ? currentFieldRect.y + FIELD_INNER_OFFSET_Y + guide.start : currentFieldRect.x + FIELD_INNER_OFFSET_X + guide.start,
                                    end: guide.orientation === 'vertical' ? currentFieldRect.y + FIELD_INNER_OFFSET_Y + guide.end : currentFieldRect.x + FIELD_INNER_OFFSET_X + guide.end,
                                  })));
                                }}
                                onDragEnd={(event: KonvaEventObject<Event>) => {
                                  if (interactionMode !== 'edit') {
                                  ...
                                  const clampedNext = clampInsideParent(next, { width: bedVm.width, height: bedVm.height }, fieldInnerSize);
                                  const snapped = snapToNeighbors(bedVm.id, clampedNext, { width: bedVm.width, height: bedVm.height }, bedViewModels);
                                  const finalClamped = clampInsideParent({ x: snapped.x, y: snapped.y }, { width: bedVm.width, height: bedVm.height }, fieldInnerSize);
                                  event.target.position({ x: currentFieldRect.x + FIELD_INNER_OFFSET_X + finalClamped.x, y: currentFieldRect.y + FIELD_INNER_OFFSET_Y + finalClamped.y });
                                  const nextLayout: BedLayoutEntry = { bed: bedVm.id, location: locationId, x: finalClamped.x, y: finalClamped.y, version: 1 };
                                  setActiveGuides([]);
                                  setLayoutsByBed((prev) => ({ ...prev, [bedVm.id]: nextLayout }));
                                  saveBedLayout(locationId, nextLayout);
                                }}
                              />
                              {visibility.showBedLabels && shouldShowBedLabel({ width: bedVm.width, height: bedVm.height }, viewport.scale) ? (
                                <Text
                                  x={currentFieldRect.x + FIELD_INNER_OFFSET_X + 4 + bedVm.x}
                                  y={currentFieldRect.y + FIELD_INNER_OFFSET_Y + 4 + bedVm.y}
                                  text={visibility.showDetailedBedLabels ? `${bedVm.name} (${bedVm.area || '-'} m²)` : bedVm.name}
                                  fontSize={12}
      ...
      <Dialog open={Boolean(selectedElement)} onClose={() => setSelectedElement(null)} fullWidth maxWidth="xs">
        <DialogTitle>{selectedElement ? t(`fields:graphical.details.${selectedElement.type}`) : ''}</DialogTitle>
        <DialogContent>
          {selectedElement ? (
            <Stack spacing={1} sx={{ mt: 1 }}>
              <Typography>{t('common:fields.name')}: {selectedElement.name}</Typography>
              <Typography>{t('fields:graphical.location')}: {selectedElement.locationName}</Typography>
              {selectedElement.parentName ? <Typography>{t('fields:graphical.parentField')}: {selectedElement.parentName}</Typography> : null}
              {selectedElement.area !== null ? <Typography>{t('fields:graphical.area')}: {selectedElement.area} m²</Typography> : null}
            </Stack>
          ) : null}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setSelectedElement(null)}>{t('common:actions.close')}</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );

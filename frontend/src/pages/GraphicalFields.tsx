...
export default function GraphicalFields({
  ...
  const [stageWidth, setStageWidth] = useState<number>(() =>
    Math.min(2200, Math.max(1200, window.innerWidth - VIEWPORT_PADDING)),
  );
        ...
        return (
          <Accordion
            ...
            onChange={(_, isExpanded) =>
              setExpanded((prev) => ({ ...prev, [location.id!]: isExpanded }))
            }
          >
                  ...
                  <Layer>
                    {defaultFieldRects.map((baseRect) => {
                      ...
                      const fieldBeds = beds.filter(
                        (bed) => bed.field === fieldId && bed.id !== undefined,
                      );
                      ...
                      return (
                        <Group key={fieldId}>
                          ...
                          <Text
                            x={fieldClamped.x + 8}
                            y={fieldClamped.y + 8}
                            text={`${baseRect.field.name} (${baseRect.field.area_sqm ?? "-"} m²)`}
                            fontStyle="bold"
                            listening={false}
                          />
                              ...
                              <Rect
                                ...
                                onDragMove={(
                                  ...
                                  const raw = {
                                    x:
                                      event.target.x() -
                                      (fieldClamped.x + FIELD_INNER_OFFSET_X),
                                    y:
                                      event.target.y() -
                                      (fieldClamped.y + FIELD_INNER_OFFSET_Y),
                                  };
                                  const clampedRaw = clampInsideParent(
                                    raw,
                                    {

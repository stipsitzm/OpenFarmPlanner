...
FIELD_ALIASES: dict[str, str] = {
    'plant_spacing_cm': 'distance_within_row_cm',
    'in_row_spacing_cm': 'distance_within_row_cm',
    'pflanzabstand_cm': 'distance_within_row_cm',
    'row_distance_cm': 'row_spacing_cm',
    'reihenabstand_cm': 'row_spacing_cm',
    'days_to_maturity': 'growth_duration_days',
    'days_to_first_harvest': 'growth_duration_days',
    'harvest_window_days': 'harvest_duration_days',
    'erntedauer_tage': 'harvest_duration_days',
    'tkg_g': 'thousand_kernel_weight_g',
    'saatmenge_g_m2': 'seed_rate_value',
    'anbaumethode': 'cultivation_type',
}
...
def _coerce_field_value(field: str, value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        if field == 'seed_rate_unit':
            ...
            if unit in {'g/m2', 'g/m²', 'g_per_m2', 'gramm/m2'}:
                return 'g_per_m2'
            if unit in {'seeds/m', 'korn/m', 'samen/m'}:
                return 'seeds/m'
            if unit in {'seeds_per_plant', 'korn/pflanze', 'samen/pflanze'}:
                return 'seeds_per_plant'
    return value


def _normalize_parsed_fields(parsed: dict[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    normalized: dict[str, Any] = {}
    alias_hits: dict[str, str] = {}

    for key, value in parsed.items():
        target = key
        if key not in ENRICH_FIELD_WHITELIST and key in FIELD_ALIASES:
            target = FIELD_ALIASES[key]
            alias_hits[key] = target

        if target in ENRICH_FIELD_WHITELIST:
            normalized[target] = _coerce_field_value(target, value)
...
def enrich_culture_data(
    ...
    normalized_parsed, alias_hits = _normalize_parsed_fields(parsed)
    ...
    for field in ENRICH_FIELD_WHITELIST:
        if field in normalized_parsed and field in effective_target_fields:
            updates[field] = normalized_parsed[field]

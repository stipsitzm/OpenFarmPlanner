...
- `GanttChartProps.onTaskDoubleClick?: (task, group) => void`, `onTaskContextMenu?: (event, task, group) => void`, `onGroupContextMenu?: (event, group) => void` — new event hooks on bars and tree rows. `onTaskDoubleClick` replaces the previously unused, dead `onTaskDoubleClick?: (task) => void` signature (it was destructured but never wired to anything). Right-click *and* a ~550ms long-press both fire the context-menu callbacks, matching the app's established touch context-menu behavior.
